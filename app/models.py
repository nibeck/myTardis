...
class TARDIS:
    ...
    def colorWipe(self, color, delay):
        """Wipe color across display a pixel at a time."""
        for i in range(globalLightStrip.numPixels()):
            globalLightStrip.setPixelColor(i, color)
            globalLightStrip.show()
            time.sleep(delay)

    def rainbowChase(self, delay):
        for j in range(256):
            for q in range(3):
                for i in range(0, globalLightStrip.numPixels(), 3):
                    globalLightStrip.setPixelColor(
                        i + q, color_utils.wheel((i + j) % 255)
                    )
                globalLightStrip.show()
                time.sleep(delay)
                for i in range(0, globalLightStrip.numPixels(), 3):
                    globalLightStrip.setPixelColor(i + q, 0)

    def rainbow(self, cycles, delay):
        for cycle in range(cycles):
            self.frontWindow.color = random_color_generator()
            self.backWindow.color = random_color_generator()
            self.rtWindow.color = random_color_generator()
            self.leftWindow.color = random_color_generator()
            time.sleep(delay)
